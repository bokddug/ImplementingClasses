...
class Point(object):
    ...
    def move_to(self,x,y):
        self.travel+= self.get_distance_from(Point(x,y))
        self.x =x
        self.y =y
        self.moves=self.moves+1

    def move_by(self,dx,dy):
        self.travel += self.get_distance_from(Point(self.x+dx,self.y+dy))
        self.x= self.x+dx
        self.y= self.y+dy
        self.moves=self.moves+1
    ...
    def get_distance_from(self,self1):
        S= Point(self.x,self.y)
        S1= Point(self1.x,self1.y)
        import math
        distance= math.sqrt(((S1.x-S.x)**2)+((S1.y-S.y)**2))
        return distance
    ...
    def halfway_to(self,p2):
        H =Point(((self.x+p2.x)/2),((self.y+p2.y)/2))
        return H
